...
def get_ruling_ids_ascending(latest_web_id: int, latest_id_in_db: int, ruling_ids_in_db: set) -> list:
    ...
    highest_ruling_id = max(to_be_scraped_case_ids)
    while len(to_be_scraped_case_ids) < settings.SCRAPER_SINGLE_RUN_CASE_COUNT:
        for i in range(highest_ruling_id + 1, highest_ruling_id + settings.SCRAPER_SINGLE_RUN_CASE_COUNT):
            highest_ruling_id = i
            to_be_scraped_case_ids.add(highest_ruling_id)

        to_be_scraped_case_ids.difference_update(ruling_ids_in_db)

    ruling_ids_in_correct_batch_size = sorted(to_be_scraped_case_ids, key=int)[
