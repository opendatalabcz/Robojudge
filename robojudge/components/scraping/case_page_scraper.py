...
class CasePageScraper:
    ...
    async def scrape_case(self) -> Case | None:
        try:
            ...
            logger.debug(f"Scraped case: {self.case_id}")
            ...
            logger.error(f'Error while scraping case "{self.case_id}": {e}')
        ...
        async with async_playwright() as pw:
            ...
            try:
                await page.wait_for_selector(
                    'div[id="PrintDiv"]', timeout=settings.SCRAPER_TIMEOUT
                )
            except PlaywrightTimeoutError:
                logger.debug(f"Timeout or no case with {self.case_id}.")
                return ""
    ...
    def extract_metadata(self):
        ...
        if metadata.get(CaseMetadataAttributes.KEYWORDS, None):
            metadata[CaseMetadataAttributes.KEYWORDS] = metadata[
                CaseMetadataAttributes.KEYWORDS
            ].split(", ")

        if metadata.get(CaseMetadataAttributes.REGULATIONS_MENTIONED, None):
            metadata[CaseMetadataAttributes.REGULATIONS_MENTIONED] = metadata[
                CaseMetadataAttributes.REGULATIONS_MENTIONED
            ].split(", ")
    ...
    def extract_text(self) -> tuple[str, str]:
        ...
        reasoning = "\n".join(
            re.sub(CasePageScraper.PARAGRAPH_NUM_REGEX, "", chunk)
