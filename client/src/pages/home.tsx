...
// TODO: loading when cases are searched for
...
export function Home() {
  ...
  return (
    <Container fluid style={{ padding: "1rem" }}>
        ...
        <Card style={styles.searchCard}>
          ...
          <Card.Body>
            ...
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              ...
              <div>
                <Button variant="primary" onClick={searchForCases}>
                  Hledat
                </Button>
              </div>
            </div>
      ...
      <Row>
        {/* TODO: arrows to scroll through the cases */}
        {cases.map(({ id, summary, metadata }) => (
          <Col xs={12} lg={4} key={id}>
            <Card style={styles.caseCard}>
              ...
              <Card.Subtitle>{(metadata?.court as string) ?? ""}</Card.Subtitle>
              <span>
                {((metadata?.keywords as Array<string>) ?? []).join(", ")}
              </span>
              <hr />
              <Card.Text>{summary}</Card.Text>
              <a href={`${process.env.REACT_APP_JUSTICE_URL}/${id}`} target="_blank" rel="noopener noreferrer">
                Celý text
              </a>
            </Card>
          </Col>
