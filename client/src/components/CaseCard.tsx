...
import { Button, Card, CardActions, CardContent, Collapse, IconButton, IconButtonProps, LinearProgress, TextField, Typography } from "@mui/material";
  ...
  const sendCaseQuestion = async () => {
    try {
      ...
      const { data } = await axios.post(
        `${process.env.REACT_APP_SERVER_URL ?? "http://localhost:4000"}/cases/question`,
        {
      ...
      console.error(err);
      triggerAlert("Při dotazování nastala chyba. Opakujte prosím akci za chvíli.");
    } finally {
      setIsLoading(false);
    }
  }
      ...
      <Card style={styles.caseCard} variant="outlined">
        ...
        <CardContent>
          <div style={styles.caseCardHeader}>
            <Typography variant="h5">{(courtCase.title as string) ?? ""}</Typography>
            <span>
        ...
        <Collapse in={isCardExpanded} timeout="auto" unmountOnExit>
          <Grid2 style={styles.questionContainer}>
            <TextField
              ...
              onKeyDown={(e) => e.key === 'Enter' && caseQuestion ? sendCaseQuestion() : null}
            />
            <div>
              <Button
                variant="outlined"
