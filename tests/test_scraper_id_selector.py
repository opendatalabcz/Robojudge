...
from robojudge.components.scraping.ruling_ids_selector import SimpleRulingIdSelector
...
def test_ascending_order_empty_db():
    ...
    ruling_ids = SimpleRulingIdSelector.get_ruling_ids_ascending(
    ...
    assert set(ruling_ids) == set(
        list(
            range(
                settings.OLDEST_KNOWN_CASE_ID + 1,
                settings.OLDEST_KNOWN_CASE_ID
...
def test_descending_order_empty_db():
    ...
    ruling_ids = SimpleRulingIdSelector.get_ruling_ids_descending(
...
def test_descending_order_continuing_from_db():
    ...
    assert set(ruling_ids) == set(
        list(
            range(
                latest_web_id - settings.SCRAPER_SINGLE_RUN_CASE_COUNT,
                latest_web_id - 2 * settings.SCRAPER_SINGLE_RUN_CASE_COUNT,
                -1,
            )
        )
    )
    ...
    assert set(ruling_ids) == set(
        [
            ...
            *list(
                range(
                    latest_web_id - settings.SCRAPER_SINGLE_RUN_CASE_COUNT,
                    latest_web_id - (2 * settings.SCRAPER_SINGLE_RUN_CASE_COUNT - 2),
                    -1,
                )
            ),
        ]
...
def test_descending_order_newest_web_id_and_continuing_from_db():
    ...
    ruling_ids_in_db = set(
        range(
            latest_in_in_db,
            latest_in_in_db - settings.SCRAPER_SINGLE_RUN_CASE_COUNT,
            -1,
        )
    )
    ...
    assert set(ruling_ids) == set(
        [
            *list(range(latest_web_id, latest_web_id - 2, -1)),
            *list(
                range(
                    latest_in_in_db - settings.SCRAPER_SINGLE_RUN_CASE_COUNT,
                    latest_in_in_db - (2 * settings.SCRAPER_SINGLE_RUN_CASE_COUNT - 2),
                    -1,
                )
            ),
        ]
...
def test_descending_order_reached_oldest_case_id():
    ...
    ruling_ids_in_db = set(range(latest_web_id, latest_in_in_db, -1))
